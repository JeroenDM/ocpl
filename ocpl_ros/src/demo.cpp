...
int main(int argc, char** argv)
{
    ...
    for (auto node : path)
    {
        std::cout << (*node) << " dist: " << node->dist << " cost: " << node->cost << "\n";
        if (robot.isInCollision(node->data))
            robot.plot(rviz.visual_tools_, node->data, rviz_visual_tools::RED);
        else
            robot.plot(rviz.visual_tools_, node->data, rviz_visual_tools::GREEN);
        ros::Duration(0.5).sleep();
    }

    // int i{ 0 };
    // for (std::vector<JointPositions>& col : graph_data)
    // {
    //     std::cout << "Waypoint: " << i << "\n";
    //     i++;
    //     for (JointPositions& q : col)
